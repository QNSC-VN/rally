...
export const EnvSchema = z
  ...
  .superRefine((env, ctx) => {
    // BFF mode turns the API into a confidential OIDC client; refuse to boot in
    // that mode unless every credential the flow needs is present, so a
    // half-configured deployment fails fast at startup instead of at login time.
    if (env.AUTH_MODE !== 'bff') return;
    const requiredForBff = [
      'ENTRA_TENANT_ID',
      'ENTRA_CLIENT_ID',
      'ENTRA_CLIENT_SECRET',
      'ENTRA_REDIRECT_URI',
    ] as const;
    for (const key of requiredForBff) {
      if (!env[key]) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [key],
          message: `${key} is required when AUTH_MODE=bff`,
        });
      }
    }
  });
